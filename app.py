# app.py — หน้าเดียว: ด้านบน (เจ้าหน้าที่) / ด้านล่าง (พื้นที่ประกาศ)
...
MODEL_PATH = "earthquake_model.pkl"      # จาก train_model.py
ENC_PATH   = "label_encoder.pkl"          # จาก train_model.py
ANN_PATH   = "storage/public_announcements.json"

st.title("🌎 ระบบแจ้งเตือนแผ่นดินไหว (หน้าเดียว)")
st.caption("กรอกค่าหรือเลือกเหตุการณ์ → ทำนายด้วย AI → เผยแพร่ประกาศ (โชว์ด้านล่าง)")

# --- ตรวจไฟล์ที่จำเป็น ---
missing = []
for p in [DATA_PATH, MODEL_PATH, ENC_PATH]:
...
# --- ส่วนเจ้าหน้าที่ ---
st.subheader("👮‍♀️ ส่วนเจ้าหน้าที่: เลือกเหตุการณ์หรือกรอกค่าเอง แล้วกดทำนาย")
latest = df.tail(200).reset_index(drop=True)

left, right = st.columns([1,1])
with left:
    st.markdown("**เลือกเหตุการณ์จากตาราง (200 แถวล่าสุด)**")
...
with right:
    st.markdown("**กรอกค่าเอง (จะทับค่าจากแถวที่เลือกได้)**")
...
if st.button("🧠 ทำนายด้วย AI", use_container_width=True):
    ...
    st.session_state["last_pred"] = {
        "risk": y_label, "inputs": inputs.iloc[0].to_dict(),
        "region": str(row.get("place","Affected area"))
    }
